...
def process_video(video_path, output_path='output.avi'):
    ...
    while cap.isOpened():
        ...
        for face in faces:
            ...
            if confidence > 0.95:  # Adjust confidence threshold if needed
                ...
                results.append(class_label)  # Store each prediction
    ...
    if results:
        final_result = max(set(results), key=results.count)   # Get the most frequent prediction
        print(f"\nFinal Video Classification: {final_result}")
    else:
